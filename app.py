...
st.html("""<style>
    [data-testid="stVVerticalBlockBorderBordered"] {
        ...
        border-left:5px solid #1E3A8A!important;border-radius:12px!important;
        box-shadow:0 4px 15px rgba(0,0,0,0.02)!important;padding:20px!important;}
    ...
    div[data-testid="stTabs"] button{font-size:14px!important;font-weight:600!important;color:#64748B!important;
        background-color:#F8FAFC!important;padding:10px 24px!important;margin-right:8px!important;
        border-radius:8px 8px 0px 0px!important;border:1px solid #E2E8F0!important;border-bottom:none!important;}
    div[data-testid="stTabs"] button:hover{color:#1E3A8A!important;background-color:#F1F5F9!important;}
    div[data-testid="stTabs"] button[aria-selected="true"]{color:#1E3A8A!important;background-color:#E0F2FE!important;
        border-color:#bae6fd!important;border-bottom:none!important;box-shadow:inset 0 3px 0px #0EA5E9!important;}
...
def sheets_append(onglet, valeurs):
    ...
    try:
        ...
        resp = requests.post(url,
            ...
            json={"values":[valeurs]}, timeout=15)
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            if delta < SEUIL_EN_LIGNE_SECONDES:
                statut="🟢 En ligne"; activite="Actif maintenant"
            elif delta < 300:
...
acces_autorise = (role == "Responsable" and password_correct) or \
                 (role == "Visiteur" and st.session_state.email_visiteur)

if not acces_autorise and role == "Visiteur":
    st.markdown("""<div style="background:white;padding:20px;border-radius:12px;border-left:5px solid #0EA5E9;
...
if acces_autorise:
    ...
    val_alert = len(df_planning[df_planning["Statut"].astype(str).str.strip().str.lower()=="non conforme"]) \
                if not df_planning.empty and "Statut" in df_planning.columns else 0
        ...
        if not df_rapports.empty:
            ...
            if col_cat_r and col_date_r:
                ...
                if jour_sel and jour_sel in details_evt:
                    nb_ctrls = len(details_evt[jour_sel])
                    st.markdown(f"""<div style='margin-top:20px;padding:12px 16px;background:#F0F9FF;
                        border-left:4px solid #0EA5E9;border-radius:8px;margin-bottom:12px;'>
                        <p style='margin:0;font-size:14px;font-weight:600;color:#0C4A6E;'>
                            📋 {nb_ctrls} contrôle(s) planifié(s) le {jour_sel} {MOIS_FR[m_view]} {a_view}
                        </p></div>""", unsafe_allow_html=True)

                    nb_cols = min(nb_ctrls, 4)
                    ...
                    for idx, row_ctrl in enumerate(details_evt[jour_sel]):
                        with card_cols[idx % nb_cols]:
                            ...
                            st.markdown(f"""
                            <div style='background:white;border-top:4px solid {c_col};padding:14px;border-radius:8px;
                                ...
                                <p style='margin:0 0 2px 0;font-size:10px;color:#94a3b8;'>Dernier contrôle</p>
                                ...
                                <span style='display:inline-block;padding:3px 8px;border-radius:4px;font-size:10px;font-weight:600;
                                    background:{c_col}22;color:{c_col};'>{c_statut} — {j_txt}</span>
                            </div>""", unsafe_allow_html=True)
                elif evenements and jour_sel is None:
                    st.info("💡 Cliquez sur un jour coloré ou sur un bouton ci-dessus pour voir les détails du contrôle.")
        ...
        with tab3:
            ...
            if df_logs.empty:
                ...
                st.dataframe(df_logs,
                    column_config={"Date":st.column_config.TextColumn("📅 Date & Heure"),
